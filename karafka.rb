...
class KarafkaApp < Karafka::App
  setup do |config|
    config.kafka.seed_brokers = %w[kafka://127.0.0.1:9094]
...
Karafka.monitor.subscribe('fetcher.call.error') do |event|
  # binding.pry
...
Karafka.monitor.subscribe('connection.client.fetch_loop.error') do |event|
  # binding.pry
  puts "Oh no! An error: #{event[:error]}"
end
